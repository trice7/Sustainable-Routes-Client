  ...
  useEffect(() => {
    if (id) {
      ...
      getSingleDestination(id).then((fetchedDestination) => {
        console.log('fetchedDestination:', fetchedDestination); // Check the fetched data
        setDestination(fetchedDestination);
        console.log('location:', location); // Check the updated state
      });
    }
  }, [id]);
  const handleUpdate = () => {
      ...
      <div className="content">
        ...
        <div className="card-container">
          {location && location.map((p) => (
            <DestinationDetailsCard className="destination-card" key={p.id} location={p} onUpdate={handleUpdate} />
          ))}
        </div>
      </div>
    </div>
