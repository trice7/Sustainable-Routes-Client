...
function DestinationDetailsCard({ activity, destinations }) {
  ...
  return (
    <Card>
      {destinations && <Card.Img variant="top" src={destinations.image} alt={destinations.name} />}
      <Card.Body>
        <Card.Title>{destinations && destinations.name}</Card.Title>
        <p className="card-text bold">Description: {destinations && destinations.description}</p>
        <h5 className="activities-header">Activities</h5>
        <CardGroup className="d-flex align-items-center activity-cards">
          {destinations && activities.name}
